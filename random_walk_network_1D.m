...
if solve_continuous
    ...
    dt = 0.0001; % continuum model. Must have dt < 0.5*dx^2*D (CFL condition)
    dx = 0.1; % continuum model. Initial guess, may be adapted slightly
...
ylabel('n(x,t)')
if solve_continuous
    legend('disc', 'stoch', 'cont', '$n_\infty$', 'Interpreter', 'latex')
else
    legend('disc', 'stoch', '$n_\infty$', 'Interpreter', 'latex')
end
hold off
drawnow
...
for s = 1:save_iter
    ...
    if solve_continuous
        legend([pl_disc, pl_stoch, pl_cont, pl_exact], 'disc', 'stoch', 'FD', '$n_\infty$', 'Interpreter', 'latex')
    else
        legend([pl_disc, pl_stoch, pl_exact], 'disc', 'stoch', '$n_\infty$', 'Interpreter', 'latex')
    end
    hold off
